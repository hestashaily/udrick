...
//   return (
//     <div className="flex flex-col md:flex-row rounded-2xl border p-4 shadow-sm gap-4">
//       {/* Image */}
...
const statusColors: Record<Listing["status"], string> = {
  Purchased: "bg-[#E7F6EC] text-[#3DA66E]",
  Rented: "bg-[#936639]/10 text-[#936639]",
};
...
export default function RefundHistoryCard({
  ...
  const statusLabel = () => {
    const base = (
      <span className={`text-xs sm:text-sm px-3 py-1.5 rounded-full ${statusColors[status]}`}>
        {status}
      </span>
    );
    ...
    if (listing.refundStatus === "requested") {
      return (
        <div className="flex  items-center gap-2">
          {base}
          <span className="text-xs sm:text-sm px-2 py-1 rounded-full text-[#B2762F] border border-[#B2762F] bg-[#FFF6E9]">
            Requested
          </span>
        </div>
      );
    ...
    if (listing.refundStatus === "refunded") {
      return (
        <div className="flex  items-center gap-2">
          {base}
          <span className="text-xs sm:text-sm px-3 py-1 flex items-center gap-1 rounded-full text-white bg-[#34C759]">
            <CircleCheck size={16} /> Refunded
          </span>
        </div>
      );
    ...
    if (listing.refundStatus === "rejected") {
      return (
        <div className="flex items-center gap-2">
          {base}
          <span className="text-xs sm:text-sm px-3 py-1.5 rounded-full text-white bg-[#562900]">
            Rejected
          </span>
        </div>
      );
  ...
  return (
    <div className="flex flex-col md:flex-row rounded-2xl border p-4 md:p-5 shadow-sm gap-4 md:gap-6 w-full max-w-full">
      
      {/* Image */}
      <div className="relative w-full sm:w-[260px] md:w-[220px] h-[200px] sm:h-[220px] rounded-xl overflow-hidden flex-shrink-0">
        <Image
          src="/property/house.svg"
          alt="house"
          fill
      ...
      {/* Content */}
      <div className="flex flex-col justify-between space-y-2 sm:space-y-3 flex-1 w-full">
        
        {/* Header */}
        <div className="flex flex-col sm:flex-row justify-between gap-3">
          <h2 className="font-semibold text-lg sm:text-lg">{title}</h2>
          <div className="flex flex-col items-start sm:items-end gap-1">
            {statusLabel()}
            {isRented && hugReward && (
              <span className="text-xs px-2 py-0.5 bg-[#936639] text-white rounded-full">
                {hugReward}
              </span>
            )}
          </div>
        ...
        {/* Property Details */}
        <div className="text-xs sm:text-sm flex flex-wrap gap-4 ">
          <span className="inline-flex items-center gap-1"><BedSingle size={16} /> {beds} Bed</span>
          <span className="inline-flex items-center gap-1"><Bath size={16} /> {baths} Bath</span>
          <span className="inline-flex items-center gap-1"><SquareDivide size={16} /> {size}</span>
        </div>

        {/* Price + Payment */}
        <div className="flex flex-wrap justify-between items-center  gap-3">
          <p className="text-[#936639] text-lg sm:text-xl font-bold">
            {price}{" "}
            <span className="text-xs sm:text-sm font-medium text-[#5C6368]">{frequency}</span>
          </p>
          <Badge className="bg-green-100 px-3 py-1.5 text-green-600 text-xs sm:text-sm">
            Payment: {payment}
          </Badge>
        </div>
        ...
        {/* Buttons */}
        <div className="flex flex-wrap gap-3 mt-4">
          <button className="flex items-center justify-center gap-1 border rounded-2xl px-4 py-2 text-xs sm:text-sm w-full sm:w-auto hover:bg-gray-100 transition">
          ...
          {!["requested", "refunded", "rejected"].includes(listing.refundStatus || "") && (
            <button
              onClick={() => setIsDialogOpen(true)}
              className="bg-gradient-to-t from-[#A68A64] to-[#936639] text-white rounded-2xl px-4 py-2 text-xs sm:text-sm font-medium w-full sm:w-auto hover:opacity-90 transition"
            >
              Request Refund
            </button>
          )}
        ...
        {/* Rejected Reason */}
        {listing.refundStatus === "rejected" && (
          <div className="border border-[#FFCECE] my-4 rounded-2xl bg-[#FFCECE]/45 p-4">
            <div className="flex gap-3">
              <CircleX color="#FB333C" className="shrink-0" />
              <div>
                <p className="text-[#AB0712] font-semibold">Rejection Reason</p>
                <p className="text-[#AB0712] text-xs sm:text-sm leading-snug">
                  Your payment has been processed for the seller. It will be completed within 24 hours.
                </p>
              </div>
            </div>
      ...
      {/* Refund Dialog */}
      <Dialog open={isDialogOpen} onOpenChange={setIsDialogOpen}>
        <DialogContent className="w-[95%] sm:max-w-lg rounded-2xl">
          <DialogHeader>
            <Image src="/refund/refund.svg" alt="refund" height={50} width={50} className="mx-auto" />
            <DialogTitle className="text-base sm:text-xl text-center font-semibold">
              Are you sure you want to request a refund?
            </DialogTitle>
            <p className="text-xs sm:text-sm text-center text-[#516071]">
              Once you submit a refund request, it cannot be cancelled.
            </p>
          </DialogHeader>

          <div className="mt-4">
            <h1 className="font-medium text-sm sm:text-base">Reason for Refund</h1>
            <textarea
              value={reason}
              onChange={(e) => setReason(e.target.value)}
              className="w-full border rounded-xl p-3 text-sm focus:outline-none focus:ring-2 focus:ring-[#A68A64]"
              placeholder="Write your reason..."
              rows={4}
            />
          </div>
          ...
          <div className="flex flex-col sm:flex-row gap-3 mt-5">
            <button
              onClick={() => setIsDialogOpen(false)}
              className="border px-6 py-3 rounded-xl w-full hover:bg-gray-100 transition"
            >
              Cancel
            </button>
            <button
              onClick={handleConfirm}
              className="bg-gradient-to-t from-[#A68A64] to-[#936639] text-white px-6 py-3 rounded-xl w-full hover:opacity-90 transition"
            >
              Yes, Request Refund
            </button>
          </div>
